#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Traducción de la UI a inglés conciso: reemplazos literales exactos."""
import os, sys
...
def apply(rel, pairs):
    ...
    with open(p, 'w', encoding='utf-8', newline='') as f:
        f.write(c)
    print(f"OK {rel}" + (f"  MISSED: {missed}" if missed else ""))

apply('src/game/balance.ts', [
...
apply('src/ui/hud.ts', [
    ...
    (": 'REVIVE';".replace('REVIVE', 'REVIVIR'), ": 'REVIVE';"),
    ('placeholder="Escribe… (Enter)"', 'placeholder="Chat… (Enter)"'),
])

apply('src/ui/screens.ts', [
    ...
    ("'La Nova de Escarcha ralentiza al enemigo. Úsala cuando cargue.'", "'Frost Nova slows the boss. Use it on big casts.'"),
    ...
    ("Boss ${'I'.repeat(l.bossIndex + 1)}: ${BOSSES[l.bossIndex].name} ${BOSSES[l.bossIndex].title}", "Boss ${'I'.repeat(l.bossIndex + 1)}: ${BOSSES[l.bossIndex].name} — ${BOSSES[l.bossIndex].title}"),
])
